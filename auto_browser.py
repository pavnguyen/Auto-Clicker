...
def get_nbr_link_channel(number_machine):
    nbr_channel = get_params('TOTAL_CHANNEL')  # Default value
    return nbr_channel


def get_url_channel(number_machine, counter_link_channel):
    nbr_channel = get_nbr_link_channel(number_machine)
    file = int(number_machine) % int(nbr_channel)

    if file == 0:
        file = nbr_channel
    print(Back.BLACK + Fore.LIGHTMAGENTA_EX + Style.BRIGHT + '[File Opened]' +
          Style.RESET_ALL + ' ' + Back.BLACK + Fore.LIGHTCYAN_EX + str(file) + '.txt' + Style.RESET_ALL + '')
    url_channel = tuple(open('ressources\LinksClick\\' + str(file) + '.txt', 'r'))

    random_int = random.randint(counter_link_channel, counter_link_channel + get_params('LINKS_PER_CHANNEL') - 1)
    only_url = [only_url for only_url in url_channel if "http" in only_url]
    print(Back.BLACK + Fore.LIGHTMAGENTA_EX + Style.BRIGHT + '[Link Opened]' +
          Style.RESET_ALL + ' ' + Back.BLACK + Fore.LIGHTCYAN_EX + only_url[random_int] + Style.RESET_ALL)
    return only_url[random_int]
...
########################################################################################################################
def main_program():
    global counter_total_click_ads_bottom
    ...
    if len(sys.argv) > 1:
        number_machine = sys.argv[1]
        print(' ' * 27 + 'Auto Browser DAILY - AVU')
    else:
        print(Back.BLACK + Fore.LIGHTWHITE_EX + '                      [ Please enter the Machine Number: ]' +
    ...
    nbr_channel = get_nbr_link_channel(number_machine)
    counter_link_channel = 0  # IMPORTANT counter_link_channel is here!!!
    print(Back.BLACK + Fore.LIGHTCYAN_EX + Style.BRIGHT + "Number Machine: " + str(number_machine) + '' +
          Style.RESET_ALL)
    ...
    for i in range(nbr_channel):  # Boucle x channel times  | default: range(0, channel)
        ...
        if i == 0:
            print(
                Back.BLACK + Fore.LIGHTMAGENTA_EX + Style.BRIGHT + 'Please wait to check Whoer.net... ' +
                Style.RESET_ALL)
            w_load = False
            while w_load is False:
                try:
                    ...
                    sleep(3)
                    ui.WebDriverWait(browser, 8).until(lambda browser: browser.find_element_by_id('anonym_level'))
        ...
        try:
            ...
            for j in range(total_key):
                search_google(browser, main_window)  # Search Google with keywords
        ...
        url = get_url_channel(number_machine, counter_link_channel)

        switch_main_window(browser, main_window)

        print(Fore.LIGHTYELLOW_EX + Back.BLACK + ' ' * 45 + '[Counter Click Ads Bottom] => ' + Style.RESET_ALL
              + Fore.LIGHTGREEN_EX + Back.BLACK + str(counter_total_click_ads_bottom) + Style.RESET_ALL + '')
        ...
        while ads_bottom is False and counter_boucle < 3:
            ...
            try:
                ...
                if ads_bottom is True:
                    ...
                    print(Back.BLACK + Fore.LIGHTGREEN_EX + Style.BRIGHT + '[Ads Bottom] => ' + Style.RESET_ALL +
                          Back.BLACK + Fore.LIGHTYELLOW_EX + Style.BRIGHT + '[FOUNDED & CLICKED]' + Style.RESET_ALL)
                    print(
                        Fore.LIGHTYELLOW_EX + Back.BLACK + ' ' * 45 + '[Counter Click Ads Bottom] => ' + Style.RESET_ALL
                        + Fore.LIGHTGREEN_EX + Back.BLACK + str(counter_total_click_ads_bottom) + Style.RESET_ALL)
            except:
                continue
        ...
        counter_link_channel += get_params('LINKS_PER_CHANNEL')  # IMPORTANT for 3 links/
        counter_tours += 1
        ...
        print(Back.BLACK + Fore.LIGHTCYAN_EX + Style.BRIGHT + '[Duration to click ads]' + Style.RESET_ALL +
              Back.BLACK + Fore.LIGHTWHITE_EX + ' ' + str(datetime.timedelta(seconds=time.time() - start_time)) +
              '' + Style.RESET_ALL)
        print(Fore.LIGHTYELLOW_EX + Back.BLACK + ' ' * 45 + '[Counter Click Ads Bottom] => ' + Style.RESET_ALL
              + Fore.LIGHTGREEN_EX + Back.BLACK + str(counter_total_click_ads_bottom) + Style.RESET_ALL + '')
        ...
        if ads_bottom is True:
            try:
                wait_time = get_info_length_youtube(current_url) - 30
            except:
                wait_time = 180
    ...
    print(Fore.LIGHTYELLOW_EX + Back.BLACK + ' ' * 45 + '[Counter Click Ads Bottom] => ' + Style.RESET_ALL
          + Fore.LIGHTGREEN_EX + Back.BLACK + str(counter_total_click_ads_bottom) + Style.RESET_ALL + '')
    print(Back.BLACK + Fore.LIGHTRED_EX + Style.BRIGHT + 'Please wait...' + '' + Style.RESET_ALL)
...
schedule_job = get_params('SCHEDULE')
schedule.every(schedule_job).minutes.do(main_program)

while True or first_run == 0:
    first_run += 1
    if first_run == 1:
        main_program()
