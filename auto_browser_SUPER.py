...
for z in range(get_params('BOUCLE_SUPER_VIP')):
    ...
    for i in range(number_machine, nbr_channel + number_machine):
        ...
        load_url_yt = False
        count_yt = 0
        while load_url_yt is False and count_yt < 3:
            count_yt += 1

            # Get clip to click Ads
            name_clip = get_name_clip(str(file_channel))
        ...
        for index in range(5):
            if (name_clip + ' - YouTube' == linkElems[index].getText()) \
                    or (name_clip[:67] + ' ...' == linkElems[index].getText() + ' ...'):
        ...
        print(Back.BLACK + Fore.LIGHTYELLOW_EX + Style.BRIGHT + '>> URL >> ' + Style.RESET_ALL +
              Back.BLACK + Fore.LIGHTWHITE_EX + url_ytb + '' + Style.RESET_ALL)

        # Check Ads Bottom
        ads_bottom = False
        counter_boucle = 0
        timing_ads = random.randint(25, 39)
        while load_elem is True and ads_bottom is False and counter_boucle < 3:
            try:
                counter_boucle += 1
                print("Test Ads Bottom: " + str(counter_boucle))
                switch_main_window(browser, main_window)
                ads_bottom = detect_and_click_ads_bottom(browser, url_ytb, timing_ads)
